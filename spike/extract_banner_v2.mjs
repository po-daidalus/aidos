// Spike v2: real Chrome + persistent profile (less likely to get Google's "restricted view").
// Usage: node spike/extract_banner_v2.mjs "<google maps place url>"
...
try {
  ...
  // Try to open the Rezensionen tab (multiple strategies)
  for (const loc of [
  ...
  fs.writeFileSync(new URL('./out/body_v2.txt', import.meta.url).pathname, bodyText);
  await page.screenshot({ path: new URL('./out/holmes_v2.png', import.meta.url).pathname }).catch(() => {});

  const restricted = /Ansicht ist beschränkt|beschränkte Ansicht/i.test(bodyText);
  const hasReviewsTab = /Rezensionen/i.test(bodyText);
  console.log('[debug] textLen:', bodyText.length, '| restrictedView:', restricted, '| reviewsTabPresent:', hasReviewsTab);
